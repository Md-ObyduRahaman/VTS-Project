...
public class CtrlLogin {
    ...
    public ResponseEntity<String> login(@PathVariable("deviceType") Integer deviceType, @RequestParam Map<String, String> requestBody) throws JsonProcessingException {
        ...
        AESEncryptionDecryption aesCrypto = new AESEncryptionDecryption(appActiveProfile, deviceType, API_VERSION); /*// Input Validation*/
        ...
        } else reqBody.password = PasswordHashUtility.generateSHA256Hash(reqBody.password);
        VTS_LOGIN_USER vtsLoginUser = new VTS_LOGIN_USER();
        BaseResponse baseResponse = new BaseResponse();
        LoginResponse loginResponse = new LoginResponse();
        boolean isCredentialMatched = false; /*//Authenticating user*/
        ...
        if (isCredentialMatched) {
            ...
            switch (vtsLoginUser.getUSER_TYPE()) {
                case 1: {  /*//[ Mother-Acc-User ] [ User-Type = 1 ]*/
                    if (vtsLoginUser.getPARENT_PROFILE_ID().equals("0")) { /*//validation check from corporate table...?*/
        ...
        System.out.println(loginResponse.token); /*// return ResponseEntity.ok().body(aesCrypto.aesEncrypt(objectMapper.writeValueAsString(baseResponse),API_VERSION));*/
        return ResponseEntity.ok().body(objectMapper.writeValueAsString(baseResponse)); /*//account block problem*/
    }
