...
public class CtrlVehicleState {
    ...
    private ResponseEntity<String> getVehicleStateInfo(
            ...
            @PathVariable("userId") Integer userId, @PathVariable("userType") Integer userType,
        ...
        if (flag == 0) {
            count = vehicleStateRepo.findTotalNumber(mainAccountId, userType, userId, SPECIFIC_VEHICLE_ID, offSet, limit);
        }

        Optional<ArrayList<VehicleStateInfoOra>> vehicleStateInfo = vehicleStateRepo.findVehicleStateInfoInfo(mainAccountId, userType, userId, SPECIFIC_VEHICLE_ID, offSet, limit);
